...
public class ComplaintService {
    ...
    public ComplaintResponse submit(AuthenticatedUser me, CreateComplaintRequest req) {
        ...
        Complaint saved = complaintRepository.save(Complaint.create(
    ...
    @Transactional
    public ComplaintResponse senderReply(UUID id, AuthenticatedUser me, ComplaintMessageRequest req) {
        Profile sender = loadProfile(me.userId());
        Complaint complaint = loadDetailed(id);
        verifySenderOwner(complaint, me.userId());
        if (complaint.getStatus().isClosed()) {
            throw new BusinessException("COMPLAINT_CLOSED",
                    "Khiếu nại đã đóng, không thể gửi thêm tin nhắn.");
        }
        complaint.addSenderMessage(sender, req.content());
    ...
    public ComplaintResponse adminGetThread(UUID id) {
        return ComplaintResponse.fromEntity(loadDetailed(id));
    }

    @Transactional
    public ComplaintResponse adminReply(UUID id, AuthenticatedUser me, ComplaintMessageRequest req) {
        Profile admin = loadProfile(me.userId());
        Complaint complaint = loadDetailed(id);
        complaint.addAdminMessage(admin, req.content());
        return ComplaintResponse.fromEntity(complaintRepository.save(complaint));
    }
